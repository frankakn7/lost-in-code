...
export default class ChatSceneController {
    ...
    public openStoryChatViewWithoutPulling() {
        ...
        if (!this._masterSceneController.isSceneSleeping(SceneKeys.STORY_CHAT_VIEW_SCENE_KEY)) {
            ...
            this._storyChatView = new ChatViewScene(
                null,
                gameController.gameStateManager.story.history,
                SceneKeys.STORY_CHAT_VIEW_SCENE_KEY,
                false,
                false,
                // () => {
    ...
    public openTextChatView(text: string, customExitFunction: Function): void {
        ...
        const simpleChatNode: ChatFlowNode = {
            text: text,
            optionText: "Start",
            choices: new Map<string, ChatFlowNode>(),
        };

        const simpleChatFlow: ChatFlow = new ChatFlow(simpleChatNode);

        const textChatView: ChatViewScene = new ChatViewScene(
            simpleChatFlow,
            [],
            SceneKeys.SINGLE_TEXT_CHAT_VIEW_SCENE_KEY,
            false,
            true,
            customExitFunction,
        );
