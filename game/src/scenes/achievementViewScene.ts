...
export default class AchievementViewScene extends Phaser.Scene {
    ...
    public create() {
        ...
        this.add
            .image(this.cameras.main.width / 2, 600, "trophy")
            .setOrigin(0.5, 0.5)
            .setScale(13, 13);
    ...
    private _drawAchievements() {
        // let unlocked = gameController.gameStateManager.achievements.unlocked;
        ...
        for (const badge of gameController.gameStateManager.achievements.unlocked) {
            // Create a render texture to represent the achievement's background with a border.
            let rt = this.add
                .renderTexture(
                    x + this._achievementWidth / 2,
                    y + this._achievementHeight / 2,
                    this._achievementWidth + this.borderThickness,
                    this._achievementHeight + this.borderThickness,
                )
                .setOrigin(0.5, 0.5);
            rt.fill(0x00c8ff);

            // Create the achievement image and set its texture based on the unlocked achievement.
            let achievement = this.add.image(x, y, achievements[badge].texture).setOrigin(0, 0);
