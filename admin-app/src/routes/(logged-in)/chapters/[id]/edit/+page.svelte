<script lang="ts">
    ...
    const editedChapter = {...data.chapter}
    ...
    let chaptersArray = []; // Assuming chapters are stored in an array
    ...
    function fetchAndUpdateChapters(curriculumId) {
        fetchChapters(curriculumId).then(value => {
            if (value) {
                chaptersArray = value;
                chaptersArray.sort((a, b) => a.order_position - b.order_position); // Sort chapters by order_position
            }
        }).catch(errror => console.error(errror));
    }

    $: fetchAndUpdateChapters(editedChapter.curriculum_id), editedChapter.curriculum_id

    // $: updateNewChapterInArray(editedChapter)
    ...
    async function updateAllOrderPositions(chapters) {
        const editedArray = chapters.map((chapter) => chapter.id === editedChapter.id ? {...editedChapter, order_position: chapter.order_position} : chapter);
        await Promise.all(editedArray.map(updateChapterOrderPosition));
        return
    }
    ...
    function dragOverHandler(event, index) {
        ...
        if (index !== draggedIndex) {
            ...
            for (let i in chaptersArray) {
                ...
                // if (chaptersArray[i].id == editedChapter.id) {
                //     editedChapter.order_position = +i + 1;
                // }
            }
            draggedIndex = index;
        }
...
<form on:submit|preventDefault={handleSubmit}>
    <table>
        <tr>
            ...
            <td>
                <input id="name" bind:value={editedChapter.name} type="text" required/>
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                <select id="curriculumId" bind:value={editedChapter.curriculum_id} required>
        ...
        <div id="drag-container">
            {#each chaptersArray as chapter, index (chapter.id)}
                <div
                        ...
                        class:newChapterDragContainer={chapter.id === editedChapter.id}
                >
                    {index + 1}:
                    {chapter.name}
                </div>
    ...
    <div class="preview-container">
        <CollapseContainer heading="Preview">
            <div class="preview">
                ...
                <MaterialPreview text={editedChapter.material}/>
            </div>
        </CollapseContainer>
    </div>
